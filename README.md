...
The original repository had to be removed by the original author - we are now continuing development on this repository here.
This is the only official repository and is maintained by the community.
 **Join the chennel me [here](https://whatsapp.com/channel/0029VbATY3LCHDypsQt6LB2p)**
...
```
yarn add alan/baileys
```

Use the edge version (no guarantee of stability, but latest fixes + features)
```
yarn add github:alan/Baileys
```
...
``` ts
import makeWASocket, { DisconnectReason } from 'alan/baileys'
...
``` ts
import makeWASocket, { BufferJSON, useMultiFileAuthState } from 'alan/baileys'
...
``` ts
import makeWASocket, { makeInMemoryStore } from 'alan/baileys'
...
``` ts
import { MessageType, MessageOptions, Mimetype } from 'alan/baileys'
...
``` ts
// send a link
const sentMsg  = await sock.sendMessage(id, { text: 'Hi, this was sent using https://github.com/RAMALANHEKING99/baileys' })
```
...
- **MiscGenerationOptions**: some extra info about the message. It can have the following __optional__ values:
    ...
    const info: MessageOptions = {
        ...
        mimetype: Mimetype.pdf, /* (for media messages) specify the type of media (optional for all media types except documents),
                                    import {Mimetype} from 'alan/baileys'
                                */
...
import { downloadMediaMessage } from 'alan/baileys'
